...
import cn.ideasoft.yuqing.analysis.*;
...
import cn.ideasoft.yuqing.crawl.CrawlDb;
import cn.ideasoft.yuqing.crawl.Inlinks;
import cn.ideasoft.yuqing.crawl.LinkDb;
...
public class Indexer extends ToolBase implements Reducer, Mapper {
    ...
    public RecordWriter getRecordWriter(final FileSystem fs, JobConf job,
      ...
      final IndexWriter writer =                  // build locally first
        new IndexWriter(fs.startLocalOutput(perm, temp).toString(),
                        new YuQingDocumentAnalyzer(job), true);
      ...
      writer.setSimilarity(new YuQingSimilarity());
      ...
      return new RecordWriter() {
          ...
          public void write(WritableComparable key, Writable value)
            ...
            YuQingAnalyzer analyzer = factory.get(doc.get("lang"));
  ...
  public void reduce(WritableComparable key, Iterator values,
    ...
    doc.add(new Field("segment", metadata.get(YuQing.SEGMENT_NAME_KEY),
            Field.Store.YES, Field.Index.NO));

    // add digest, used by dedup
    doc.add(new Field("digest", metadata.get(YuQing.SIGNATURE_KEY),
            Field.Store.YES, Field.Index.NO));
...
//     }
  ...
  public static void main(String[] args) throws Exception {
    int res = new Indexer().doMain(YuQingConfiguration.create(), args);
    System.exit(res);
  }
