...
public class OpenSearchServlet extends HttpServlet {
  ...
  private YuQingBean bean;
  private Configuration conf;

  public void init(ServletConfig config) throws ServletException {
    try {
      this.conf = YuQingConfiguration.get(config.getServletContext());
      bean = YuQingBean.get(config.getServletContext(), this.conf);
    } catch (IOException e) {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (YuQingBean.LOG.isInfoEnabled()) {
      YuQingBean.LOG.info("query request from " + request.getRemoteAddr());
    }
    ...
    if (YuQingBean.LOG.isInfoEnabled()) {
      YuQingBean.LOG.info("query: " + queryString);
      YuQingBean.LOG.info("lang: " + queryLang);
    }
    ...
    try {
      ...
      if (YuQingBean.LOG.isWarnEnabled()) {
        YuQingBean.LOG.warn("Search Error", e);
      }
      hits = new Hits(0,new Hit[0]);	
    }

    if (YuQingBean.LOG.isInfoEnabled()) {
      YuQingBean.LOG.info("total hits: " + hits.getTotal());
    }
