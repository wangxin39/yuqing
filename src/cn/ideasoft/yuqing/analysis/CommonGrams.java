...
import cn.ideasoft.yuqing.searcher.Query.*;
...
public class CommonGrams {
    ...
    try {
      ...
      while ((line = in.readLine()) != null) {
        ...
        if (line.startsWith("#") || "".equals(line)) // skip comments
          continue;
        TokenStream ts = new YuQingDocumentTokenizer(new StringReader(line));
        Token token = ts.next();
        if (token == null) {
  ...
  public static void main(String[] args) throws Exception {
    ...
    TokenStream ts = new YuQingDocumentTokenizer(new StringReader(text.toString()));
    CommonGrams commonGrams = new CommonGrams(YuQingConfiguration.create());
